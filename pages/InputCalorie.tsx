...
export default function InputCalorie(){
    ...
    const setDetectCaloriesOfMeals=(event) =>{
        ...
        const calorie = 100
        setDetectMeals(detectMeals=>[...detectMeals,{mealName:meals,mealCalorie:calorie}])
        setMeals("")
            ...
            <div className="flex-grow">
                <form onSubmit={setDetectCaloriesOfMeals}>
                    <div className="grid grid-cols-2">
                        <input placeholder="今日食べたものを入力" value={meals} type={"text"} onChange={handleMealsChange} className="border border-black"></input>
                        <div className="flex justify-center">
                            <button type="submit" className="border  border-black">入力</button>
                            <button className="border border-black" onClick={() => registMealToDatabase}>登録</button>
                        </div>
                    </div>
                </form>
                <div>今日のごはん</div>
